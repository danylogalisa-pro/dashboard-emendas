...
for row in rows:
    ...
    emendas.append({
        ...
        "diretoria": row.get("Diretoria","").strip(),
        "tipo": row.get("Forma de Execucao","").strip() or row.get("Forma de Execução","").strip(),
        "beneficiario": row.get("Beneficiario","").strip() or row.get("Beneficiário","").strip(),
        "valor": parse_moeda(row.get("Valor da Emenda")),
        "valorEmpenhado": ve,
        "valorPago": vp,
        "percentualPago": round(perc,2),
        "situacao": row.get("Situacao","").strip() or row.get("Situação","").strip(),
        "observacao": row.get("Observacao","").strip() or row.get("Observação","").strip()
    })
...
<title>Dashboard Emendas Parlamentares</title>
<style>
...
.stat-value{{font-size:1.5em;color:#667eea;font-weight:bold;margin:10px 0}}
...
<div class="container">
<header>
<h1>📊 Dashboard Emendas Parlamentares Municipais</h1>
...
<table>
<thead><tr><th>Vereador</th><th>Ano</th><th>Processo</th><th>Emenda</th><th>Objeto</th><th>Beneficiário</th><th>Empenhado</th><th>Pago</th><th>%</th><th>Situação</th></tr></thead>
...
function renderTable(){{
  let html = "";
  emendas.forEach(e=>{{
    html += `<tr><td>${{e.vereador}}</td><td>${{e.ano}}</td><td>${{e.processo}}</td><td>${{e.emenda}}</td><td>${{e.projeto}}</td><td>${{e.beneficiario}}</td><td>${{formatMoeda(e.valorEmpenhado)}}</td><td>${{formatMoeda(e.valorPago)}}</td><td>${{e.percentualPago}}%</td><td>${{e.situacao}}</td></tr>`;
  }});
